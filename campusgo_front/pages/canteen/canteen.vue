<template>
  <view class="food-list-container">
    ...
    <view class="filter-bar">
      ...
      >
        <!-- 在位置左侧添加图标 -->
        <image v-if="index === 0" class="filter-icon filter-icon-left" src="/static/location.png" mode="aspectFit"></image>
        ...
        ></uni-icons>
        <!-- 在人气、评分右侧添加图标 -->
		<image v-if="index === 1" class="filter-icon filter-icon-right" src="/static/rq.png" mode="aspectFit"></image>
        <image v-if="index === 2" class="filter-icon filter-icon-right" src="/static/pf.png" mode="aspectFit"></image>
...
const getFoodList = async (page = 1, isRefresh = false) => {
  ...
  try {
    console.log('开始请求数据...', page, 'typeId:', typeId.value);
...
const filteredList = computed(() => {
  return foodList.value.filter(item => {
    ...
    // 评分筛选 (现在索引是 2)
    if (filters.value[2].selectedIndex > 0) {
      const minRating = parseFloat(filters.value[2].options[filters.value[2].selectedIndex]);
    ...
    // 人气排序 (现在索引是 1)
    if (filters.value[1].selectedIndex > 0) {
      const sortBy = filters.value[1].options[filters.value[1].selectedIndex];
