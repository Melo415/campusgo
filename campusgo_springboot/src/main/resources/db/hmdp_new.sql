...
(
    ...
    answer_id   bigint unsigned default '0'               not null comment '回复的评论id',
...
(
    ...
    parent_id   bigint   default 0                 null comment '父评论id（一级评论为0）',
    answer_id   bigint                             null comment '回复的评论id',
    content     varchar(1024)                      not null comment '评论内容',
    liked       int      default 0                 null comment '点赞数',
    status      tinyint  default 0                 null comment '状态，0：正常，1：被举报，2：禁止查看',
    create_time datetime default CURRENT_TIMESTAMP null comment '创建时间',
    update_time datetime default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP comment '更新时间',
    constraint fk_shop_comment_shop
        foreign key (shop_id) references tb_shop (id),
    constraint fk_shop_comment_user
        foreign key (user_id) references tb_user (id)
)
    comment '店铺评论表';

create index idx_parent_id
    on tb_shop_comment (parent_id);

create index idx_shop_id
    on tb_shop_comment (shop_id);

create index idx_user_id
    on tb_shop_comment (user_id);

create table tb_user_info
(
    user_id     bigint unsigned auto_increment comment '主键，用户id'
        primary key,
